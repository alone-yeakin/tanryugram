...
async function validateMediaUpload(contentType: string, sizeBytes?: number, purpose: "post" | "profile" = "post", userId?: number) {
  const policy = await db.getMediaUploadPolicy();
  if (userId && purpose === "post") {
    const permissions = await db.getUserMediaPermissions(userId);
    if (!permissions.postsEnabled) throw new TRPCError({ code: "FORBIDDEN", message: "Posting is currently disabled for this account" });
    if (IMAGE_CONTENT_TYPES.has(contentType) && !permissions.photosEnabled) throw new TRPCError({ code: "FORBIDDEN", message: "Photo posting is currently disabled for this account" });
    if (VIDEO_CONTENT_TYPES.has(contentType) && !permissions.videosEnabled) throw new TRPCError({ code: "FORBIDDEN", message: "Video posting is currently disabled for this account" });
  }
...
export const appRouter = router({
  ...
  media: router({
    ...
    prepareUpload: protectedProcedure.input(z.object({ fileName: z.string().min(1).max(255), contentType: z.string().min(1), fileSizeBytes: z.number().int().positive().optional(), purpose: z.enum(["post", "profile"]).default("post") })).mutation(async ({ ctx, input }) => { await validateMediaUpload(input.contentType, input.fileSizeBytes, input.purpose, ctx.user.id); return storagePresignPut(`users/${ctx.user.id}/${cleanUploadFileName(input.fileName)}`, input.contentType); }),
    uploadBase64: protectedProcedure.input(z.object({ fileName: z.string().min(1).max(255), base64Data: z.string().min(1), contentType: z.string().min(1), purpose: z.enum(["post", "profile"]).default("post") })).mutation(async ({ ctx, input }) => {
      const base64 = input.base64Data.replace(/^data:.*;base64,/, "");
      const buffer = Buffer.from(base64, "base64");
