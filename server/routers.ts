...
async function validateMediaUpload(contentType: string, sizeBytes?: number, purpose: "post" | "profile" = "post") {
...
export const appRouter = router({
  ...
  media: router({
    ...
    uploadBase64: protectedProcedure.input(z.object({ fileName: z.string().min(1).max(255), base64Data: z.string().min(1), contentType: z.string().min(1), purpose: z.enum(["post", "profile"]).default("post") })).mutation(async ({ ctx, input }) => {
      const base64 = input.base64Data.replace(/^data:.*;base64,/, "");
      const buffer = Buffer.from(base64, "base64");
      await validateMediaUpload(input.contentType, buffer.length, input.purpose);
