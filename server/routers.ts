...
export const appRouter = router({
  ...
  reels: router({
    ...
    comment: protectedProcedure.input(z.object({ reelId: z.number().int().positive(), content: z.string().trim().min(1).max(500) })).mutation(({ ctx, input }) => db.createReelComment(input.reelId, ctx.user.id, input.content)),
