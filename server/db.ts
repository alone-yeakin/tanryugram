...
export async function submitContentReport(reporterId: number, input: { targetType: "account" | "post" | "video" | "reel"; targetId: number; reason: "pornography" | "child_abuse" | "dangerous" | "harassment" | "spam" | "other"; details?: string }) { const db = await getDb(); if (!db) return { id: 0, status: "pending" as const }; const existing = await db.select().from(contentReports).where(and(eq(contentReports.reporterId, reporterId), eq(contentReports.targetType, input.targetType), eq(contentReports.targetId, input.targetId), or(eq(contentReports.status, "pending"), eq(contentReports.status, "auto_hidden")))).limit(1); if (existing.length) return existing[0]; const sensitive = input.reason === "pornography" || input.reason === "child_abuse" || input.reason === "dangerous"; const status = sensitive ? "auto_hidden" as const : "pending" as const; const result = await db.insert(contentReports).values({ reporterId, ...input, details: input.details || null, status }); const id = Number(result[0]?.insertId || 0); if (input.targetType === "account") await db.update(users).set({ contentHidden: sensitive }).where(eq(users.id, input.targetId)); else await db.update(posts).set({ isHidden: sensitive }).where(eq(posts.id, input.targetId)); return { id, status }; }
...
export async function reviewContentReport(reportId: number, reviewerId: number, status: "reviewed" | "dismissed") { const db = await getDb(); if (!db) return; const report = (await db.select().from(contentReports).where(eq(contentReports.id, reportId)).limit(1))[0]; if (!report) return; await db.update(contentReports).set({ status, reviewedBy: reviewerId, reviewedAt: new Date() }).where(eq(contentReports.id, reportId)); if (status === "dismissed") { if (report.targetType === "account") await db.update(users).set({ contentHidden: false }).where(eq(users.id, report.targetId)); else await db.update(posts).set({ isHidden: false }).where(eq(posts.id, report.targetId)); } return { success: true }; }
