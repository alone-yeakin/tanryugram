...
export async function recordReelView(reelId: number, userId?: number) { const db = await getDb(); if (!db) return; await db.insert(reelViews).values({ reelId, userId: userId ?? null }); }
...
export async function createReelComment(reelId: number, userId: number, content: string) { const db = await getDb(); if (!db) return 0; const result = await db.insert(reelComments).values({ reelId, userId, content }); return Number(result[0]?.insertId || 0); }
export async function getReelComments(reelId: number) { const db = await getDb(); if (!db) return []; return db.select({ comment: reelComments, user: users }).from(reelComments).innerJoin(users, eq(reelComments.userId, users.id)).where(eq(reelComments.reelId, reelId)).orderBy(desc(reelComments.createdAt)).limit(100); }
