...
export async function getFeedPosts(limit = 20, offset = 0) { const db = await getDb(); if (!db) return []; return db.select({ post: posts, creator: publicCreatorFields }).from(posts).leftJoin(users, eq(posts.userId, users.id)).where(and(eq(posts.isHidden, false), eq(users.contentHidden, false))).orderBy(desc(posts.createdAt)).limit(limit).offset(offset); }
export async function getExplorePosts(limit = 24) { const db = await getDb(); if (!db) return []; return db.select({ post: posts, creator: publicCreatorFields }).from(posts).leftJoin(users, eq(posts.userId, users.id)).where(and(eq(posts.isHidden, false), eq(users.contentHidden, false))).orderBy(desc(posts.likesCount), desc(posts.createdAt)).limit(limit); }
...
export async function getProfilePosts(userId: number) { const db = await getDb(); if (!db) return []; const rows = await db.select({ post: posts }).from(posts).leftJoin(users, eq(posts.userId, users.id)).where(and(eq(posts.userId, userId), eq(posts.isHidden, false), eq(users.contentHidden, false))).orderBy(desc(posts.createdAt)).limit(48); return rows.map(({ post }) => post); }
export async function getUserMediaPermissions(userId: number) { const db = await getDb(); if (!db) return { userId, postsEnabled: true, photosEnabled: true, videosEnabled: false, reelsEnabled: false, storiesEnabled: true }; const existing = (await db.select().from(userMediaPermissions).where(eq(userMediaPermissions.userId, userId)).limit(1))[0]; if (existing) return existing; await db.insert(userMediaPermissions).values({ userId }); return (await db.select().from(userMediaPermissions).where(eq(userMediaPermissions.userId, userId)).limit(1))[0] ?? { userId, postsEnabled: true, photosEnabled: true, videosEnabled: false, reelsEnabled: false, storiesEnabled: true }; }
export async function getAllUserMediaPermissions() { const db = await getDb(); if (!db) return []; return db.select().from(userMediaPermissions).orderBy(desc(userMediaPermissions.updatedAt)); }
export async function createReelSubmission(userId: number, input: { mediaUrl: string; thumbnailUrl?: string; caption?: string; width: number; height: number; durationSeconds?: number }) { const db = await getDb(); if (!db) return { id: 0, status: "pending" as const }; const permissions = await getUserMediaPermissions(userId); if (!permissions.reelsEnabled) return { id: 0, status: "disabled" as const }; const ratio = input.width / input.height; if (input.width < 1 || input.height < 1 || Math.abs(ratio - 9 / 16) > 0.03) return { id: 0, status: "invalid_ratio" as const }; const result = await db.insert(reelSubmissions).values({ userId, mediaUrl: input.mediaUrl, caption: input.caption || null, width: input.width, height: input.height, durationSeconds: input.durationSeconds ?? null, thumbnailUrl: input.thumbnailUrl || null, processedMediaUrl: input.mediaUrl, processingStatus: "ready" }); const id = Number(result[0]?.insertId || 0); await addModerationAudit(userId, "reel_submitted", "reel", id, `9:16 submission ${input.width}x${input.height}`); return { id, status: "pending" as const }; }
export async function getMyReelSubmissions(userId: number) { const db = await getDb(); if (!db) return []; return db.select().from(reelSubmissions).where(eq(reelSubmissions.userId, userId)).orderBy(desc(reelSubmissions.createdAt)).limit(50); }
export async function getReelSubmissions() { const db = await getDb(); if (!db) return []; return db.select({ reel: reelSubmissions, user: users }).from(reelSubmissions).innerJoin(users, eq(reelSubmissions.userId, users.id)).orderBy(desc(reelSubmissions.createdAt)).limit(200); }
export async function toggleReelLike(reelId: number, userId: number) { const db = await getDb(); if (!db) return { liked: false, count: 0 }; const existing = (await db.select().from(reelLikes).where(and(eq(reelLikes.reelId, reelId), eq(reelLikes.userId, userId))).limit(1))[0]; if (existing) await db.delete(reelLikes).where(eq(reelLikes.id, existing.id)); else { await db.insert(reelLikes).values({ reelId, userId }); await updateDailyReelAnalytics(reelId, "likes", 1); } const rows = await db.select({ count: sql<number>`COUNT(*)` }).from(reelLikes).where(eq(reelLikes.reelId, reelId)); return { liked: !existing, count: Number(rows[0]?.count ?? 0) }; }
export async function getReelLikeCounts(reelIds: number[]) { const db = await getDb(); if (!db || !reelIds.length) return {}; const rows = await db.select({ reelId: reelLikes.reelId, count: sql<number>`COUNT(*)` }).from(reelLikes).where(inArray(reelLikes.reelId, reelIds)).groupBy(reelLikes.reelId); return Object.fromEntries(rows.map((row) => [row.reelId, Number(row.count)])); }
export async function getReelPromotions() { const db = await getDb(); if (!db) return []; return db.select().from(reelPromotions).orderBy(desc(reelPromotions.createdAt)).limit(100); }
export async function setReelPromotion(ownerId: number, input: { reelId: number; priority: number; status: "active" | "paused" | "ended"; endsAt?: Date | null }) { const db = await getDb(); if (!db) return { success: false }; const existing = (await db.select().from(reelPromotions).where(eq(reelPromotions.reelId, input.reelId)).limit(1))[0]; if (existing) await db.update(reelPromotions).set({ priority: input.priority, status: input.status, endsAt: input.endsAt ?? null }).where(eq(reelPromotions.id, existing.id)); else await db.insert(reelPromotions).values({ ownerId, reelId: input.reelId, priority: input.priority, status: input.status, endsAt: input.endsAt ?? null }); await addModerationAudit(ownerId, "reel_promotion_updated", "reel", input.reelId, `${input.status} priority ${input.priority}`); return { success: true }; }
export async function recordReelView(reelId: number, userId?: number) { const db = await getDb(); if (!db) return; await db.insert(reelViews).values({ reelId, userId: userId ?? null }); await updateDailyReelAnalytics(reelId, "views", 1); }
async function updateDailyReelAnalytics(reelId: number, type: "views" | "likes" | "comments", delta: number) { const db = await getDb(); if (!db) return; const today = new Date(); today.setUTCHours(0, 0, 0, 0); const existing = (await db.select().from(dailyReelAnalytics).where(and(eq(dailyReelAnalytics.reelId, reelId), eq(dailyReelAnalytics.date, today))).limit(1))[0]; if (existing) await db.update(dailyReelAnalytics).set({ [type]: sql`${dailyReelAnalytics[type]} + ${delta}` }).where(eq(dailyReelAnalytics.id, existing.id)); else await db.insert(dailyReelAnalytics).values({ reelId, date: today, [type]: delta }); }
export async function getReelSevenDayTrends(reelId: number) { const db = await getDb(); if (!db) return []; const sevenDaysAgo = new Date(); sevenDaysAgo.setUTCDate(sevenDaysAgo.getUTCDate() - 7); sevenDaysAgo.setUTCHours(0, 0, 0, 0); return db.select().from(dailyReelAnalytics).where(and(eq(dailyReelAnalytics.reelId, reelId), lt(dailyReelAnalytics.date, new Date()), sql`${dailyReelAnalytics.date} >= ${sevenDaysAgo}`)).orderBy(desc(dailyReelAnalytics.date)).limit(7); }
export async function toggleReelBookmark(reelId: number, userId: number) { const db = await getDb(); if (!db) return { bookmarked: false }; const existing = (await db.select().from(reelBookmarks).where(and(eq(reelBookmarks.reelId, reelId), eq(reelBookmarks.userId, userId))).limit(1))[0]; if (existing) await db.delete(reelBookmarks).where(eq(reelBookmarks.id, existing.id)); else await db.insert(reelBookmarks).values({ reelId, userId }); return { bookmarked: !existing }; }
export async function getMySavedReels(userId: number) { const db = await getDb(); if (!db) return []; return db.select({ reel: reelSubmissions, user: users }).from(reelSubmissions).innerJoin(users, eq(reelSubmissions.userId, users.id)).innerJoin(reelBookmarks, eq(reelBookmarks.reelId, reelSubmissions.id)).where(eq(reelBookmarks.userId, userId)).orderBy(desc(reelBookmarks.createdAt)).limit(100); }
export async function createReelComment(reelId: number, userId: number, content: string, parentId?: number) { const db = await getDb(); if (!db) return 0; const result = await db.insert(reelComments).values({ reelId, userId, content, parentId: parentId ?? null }); await updateDailyReelAnalytics(reelId, "comments", 1); return Number(result[0]?.insertId || 0); }
export async function toggleReelCommentLike(commentId: number, userId: number) { const db = await getDb(); if (!db) return { liked: false }; const existing = (await db.select().from(reelCommentLikes).where(and(eq(reelCommentLikes.commentId, commentId), eq(reelCommentLikes.userId, userId))).limit(1))[0]; if (existing) await db.delete(reelCommentLikes).where(eq(reelCommentLikes.id, existing.id)); else await db.insert(reelCommentLikes).values({ commentId, userId }); return { liked: !existing }; }
export async function getReelComments(reelId: number) { const db = await getDb(); if (!db) return []; const rows = await db.select({ comment: reelComments, user: users }).from(reelComments).innerJoin(users, eq(reelComments.userId, users.id)).where(eq(reelComments.reelId, reelId)).orderBy(desc(reelComments.createdAt)).limit(200); const commentIds = rows.map(r => r.comment.id); const likes = commentIds.length ? await db.select({ commentId: reelCommentLikes.commentId, count: sql<number>`COUNT(*)` }).from(reelCommentLikes).where(inArray(reelCommentLikes.commentId, commentIds)).groupBy(reelCommentLikes.commentId) : []; const likesMap = Object.fromEntries(likes.map(l => [l.commentId, Number(l.count)])); return rows.map(r => ({ ...r, likeCount: likesMap[r.comment.id] || 0 })); }
export async function getReelEngagementAnalytics(reelIds: number[]) { const db = await getDb(); if (!db || !reelIds.length) return {}; const [views, likesCount, commentsCount] = await Promise.all([db.select({ reelId: reelViews.reelId, count: sql<number>`COUNT(*)` }).from(reelViews).where(inArray(reelViews.reelId, reelIds)).groupBy(reelViews.reelId), db.select({ reelId: reelLikes.reelId, count: sql<number>`COUNT(*)` }).from(reelLikes).where(inArray(reelLikes.reelId, reelIds)).groupBy(reelLikes.reelId), db.select({ reelId: reelComments.reelId, count: sql<number>`COUNT(*)` }).from(reelComments).where(inArray(reelComments.reelId, reelIds)).groupBy(reelComments.reelId)]); const stats: Record<number, { views: number; likes: number; comments: number }> = {}; reelIds.forEach(id => stats[id] = { views: 0, likes: 0, comments: 0 }); views.forEach(r => stats[r.reelId].views = Number(r.count)); likesCount.forEach(r => stats[r.reelId].likes = Number(r.count)); commentsCount.forEach(r => stats[r.reelId].comments = Number(r.count)); return stats; }
export async function getApprovedReels(limit = 6, cursor?: number) { const db = await getDb(); if (!db) return { items: [], nextCursor: null }; const rows = await db.select({ reel: reelSubmissions, user: users }).from(reelSubmissions).innerJoin(users, eq(reelSubmissions.userId, users.id)).where(and(eq(reelSubmissions.status, "approved"), eq(reelSubmissions.processingStatus, "ready"), eq(users.contentHidden, false), cursor ? lt(reelSubmissions.id, cursor) : undefined)).orderBy(desc(reelSubmissions.id)).limit(limit + 1); const hasMore = rows.length > limit; let items = hasMore ? rows.slice(0, limit) : rows; if (!cursor) { const promoted = await db.select({ reel: reelSubmissions, user: users }).from(reelSubmissions).innerJoin(users, eq(reelSubmissions.userId, users.id)).innerJoin(reelPromotions, eq(reelPromotions.reelId, reelSubmissions.id)).where(and(eq(reelPromotions.status, "active"), eq(reelSubmissions.status, "approved"), eq(reelSubmissions.processingStatus, "ready"), eq(users.contentHidden, false))).orderBy(desc(reelPromotions.priority), desc(reelPromotions.createdAt)).limit(3); const existingIds = new Set(items.map(i => i.reel.id)); const uniquePromoted = promoted.filter(p => !existingIds.has(p.reel.id)).map(p => ({ ...p, isPromoted: true })); items = [...uniquePromoted, ...items]; } return { items, nextCursor: hasMore ? (rows[limit]?.reel.id ?? null) : null }; }
export async function reviewReel(reelId: number, reviewerId: number, status: "approved" | "rejected", reviewNote?: string) { const db = await getDb(); if (!db) return { success: false }; const existing = (await db.select().from(reelSubmissions).where(eq(reelSubmissions.id, reelId)).limit(1))[0]; if (!existing) return { success: false }; await db.update(reelSubmissions).set({ status, reviewNote: reviewNote || null, reviewedBy: reviewerId, reviewedAt: new Date() }).where(eq(reelSubmissions.id, reelId)); await addModerationAudit(reviewerId, `reel_${status}`, "reel", reelId, reviewNote || null); await createNotification({ userId: existing.userId, actorId: reviewerId, type: status === "approved" ? "reel_approved" : "reel_rejected", targetId: reelId, content: status === "approved" ? "your Reel was approved and is now in Explore" : `your Reel was not approved${reviewNote ? `: ${reviewNote}` : ""}` }); return { success: true }; }
export async function addModerationAudit(actorId: number | null, action: string, targetType: string, targetId?: number, details?: string | null) { const db = await getDb(); if (!db) return; await db.insert(moderationAuditLog).values({ actorId, action, targetType, targetId: targetId ?? null, details: details || null }); }
export async function getModerationAuditLog() { const db = await getDb(); if (!db) return []; return db.select().from(moderationAuditLog).orderBy(desc(moderationAuditLog.createdAt)).limit(300); }
export async function getMyContentAppeals(appellantId: number) { const db = await getDb(); if (!db) return []; return db.select().from(contentAppeals).where(eq(contentAppeals.appellantId, appellantId)).orderBy(desc(contentAppeals.createdAt)).limit(50); }
export async function getContentAppeals() { const db = await getDb(); if (!db) return []; return db.select({ appeal: contentAppeals, user: users }).from(contentAppeals).innerJoin(users, eq(contentAppeals.appellantId, users.id)).orderBy(desc(contentAppeals.createdAt)).limit(200); }
export async function createContentAppeal(appellantId: number, input: { targetType: "account" | "post" | "video" | "reel"; targetId: number; reason: string }) { const db = await getDb(); if (!db) return { id: 0, status: "pending" as const }; const existing = await db.select().from(contentAppeals).where(and(eq(contentAppeals.appellantId, appellantId), eq(contentAppeals.targetType, input.targetType), eq(contentAppeals.targetId, input.targetId), eq(contentAppeals.status, "pending"))).limit(1); if (existing.length) return existing[0]; const result = await db.insert(contentAppeals).values({ appellantId, ...input }); const id = Number(result[0]?.insertId || 0); await addModerationAudit(appellantId, "appeal_submitted", input.targetType, input.targetId, input.reason); return { id, status: "pending" as const }; }
export async function reviewContentAppeal(appealId: number, reviewerId: number, status: "approved" | "rejected", response?: string) { const db = await getDb(); if (!db) return { success: false }; const appeal = (await db.select().from(contentAppeals).where(eq(contentAppeals.id, appealId)).limit(1))[0]; if (!appeal) return { success: false }; await db.update(contentAppeals).set({ status, response: response || null, reviewedBy: reviewerId, reviewedAt: new Date() }).where(eq(contentAppeals.id, appealId)); if (status === "approved") { if (appeal.targetType === "account") await db.update(users).set({ contentHidden: false }).where(eq(users.id, appeal.targetId)); else if (appeal.targetType === "reel") await db.update(reelSubmissions).set({ status: "approved" }).where(eq(reelSubmissions.id, appeal.targetId)); else await db.update(posts).set({ isHidden: false }).where(eq(posts.id, appeal.targetId)); } await addModerationAudit(reviewerId, `appeal_${status}`, appeal.targetType, appeal.targetId, response || null); await createNotification({ userId: appeal.appellantId, actorId: reviewerId, type: status === "approved" ? "appeal_approved" : "appeal_rejected", targetId: appeal.targetId, content: status === "approved" ? "your appeal was approved and the content was restored" : "your appeal was reviewed and the original decision remains" }); return { success: true }; }
async function consumeReportRateLimit(reporterId: number) { const db = await getDb(); if (!db) return true; const now = new Date(); const existing = (await db.select().from(contentReportRateLimits).where(eq(contentReportRateLimits.reporterId, reporterId)).limit(1))[0]; if (!existing || now.getTime() - new Date(existing.windowStartedAt).getTime() >= 60 * 60 * 1000) { if (existing) await db.update(contentReportRateLimits).set({ windowStartedAt: now, reportCount: 1 }).where(eq(contentReportRateLimits.reporterId, reporterId)); else await db.insert(contentReportRateLimits).values({ reporterId, windowStartedAt: now, reportCount: 1 }); return true; } if (existing.reportCount >= 5) return false; await db.update(contentReportRateLimits).set({ reportCount: existing.reportCount + 1 }).where(eq(contentReportRateLimits.reporterId, reporterId)); return true; }
export async function updateUserMediaPermissions(updatedBy: number, userId: number, input: { postsEnabled?: boolean; photosEnabled?: boolean; videosEnabled?: boolean; reelsEnabled?: boolean; storiesEnabled?: boolean }) { const db = await getDb(); if (!db) return getUserMediaPermissions(userId); const existing = (await db.select().from(userMediaPermissions).where(eq(userMediaPermissions.userId, userId)).limit(1))[0]; if (existing) await db.update(userMediaPermissions).set({ ...input, updatedBy }).where(eq(userMediaPermissions.userId, userId)); else await db.insert(userMediaPermissions).values({ userId, updatedBy, ...input }); return getUserMediaPermissions(userId); }
export async function submitContentReport(reporterId: number, input: { targetType: "account" | "post" | "video" | "reel"; targetId: number; reason: "pornography" | "child_abuse" | "dangerous" | "harassment" | "spam" | "other"; details?: string }) { const db = await getDb(); if (!db) return { id: 0, status: "pending" as const }; if (!(await consumeReportRateLimit(reporterId))) return { id: 0, status: "rate_limited" as const }; const existing = await db.select().from(contentReports).where(and(eq(contentReports.reporterId, reporterId), eq(contentReports.targetType, input.targetType), eq(contentReports.targetId, input.targetId), or(eq(contentReports.status, "pending"), eq(contentReports.status, "auto_hidden")))).limit(1); if (existing.length) return existing[0]; const sensitive = input.reason === "pornography" || input.reason === "child_abuse" || input.reason === "dangerous"; const status = sensitive ? "auto_hidden" as const : "pending" as const; const result = await db.insert(contentReports).values({ reporterId, ...input, details: input.details || null, status }); const id = Number(result[0]?.insertId || 0); if (input.targetType === "account") await db.update(users).set({ contentHidden: sensitive }).where(eq(users.id, input.targetId)); else if (input.targetType === "reel") await db.update(reelSubmissions).set({ status: sensitive ? "rejected" : "pending" }).where(eq(reelSubmissions.id, input.targetId)); else await db.update(posts).set({ isHidden: sensitive }).where(eq(posts.id, input.targetId)); await addModerationAudit(reporterId, sensitive ? "report_auto_hidden" : "report_submitted", input.targetType, input.targetId, input.reason); return { id, status }; }
export async function getContentReports() { const db = await getDb(); if (!db) return []; return db.select({ report: contentReports, reporter: users }).from(contentReports).innerJoin(users, eq(contentReports.reporterId, users.id)).orderBy(desc(contentReports.createdAt)).limit(200); }
export async function reviewContentReport(reportId: number, reviewerId: number, status: "reviewed" | "dismissed") { const db = await getDb(); if (!db) return; const report = (await db.select().from(contentReports).where(eq(contentReports.id, reportId)).limit(1))[0]; if (!report) return; await db.update(contentReports).set({ status, reviewedBy: reviewerId, reviewedAt: new Date() }).where(eq(contentReports.id, reportId)); if (status === "dismissed") { if (report.targetType === "account") await db.update(users).set({ contentHidden: false }).where(eq(users.id, report.targetId)); else if (report.targetType === "reel") await db.update(reelSubmissions).set({ status: "pending" }).where(eq(reelSubmissions.id, report.targetId)); else await db.update(posts).set({ isHidden: false }).where(eq(posts.id, report.targetId)); } await addModerationAudit(reviewerId, `report_${status}`, report.targetType, report.targetId, report.reason); return { success: true }; }
...
export async function createNotification(input: { userId: number; actorId: number; type: "like" | "comment" | "follow" | "message" | "tip" | "subscribe" | "reel_approved" | "reel_rejected" | "appeal_approved" | "appeal_rejected"; targetId?: number; content: string }) { const db = await getDb(); if (!db) return; await db.insert(notifications).values(input); }
