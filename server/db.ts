...
export async function createReelSubmission(userId: number, input: { mediaUrl: string; caption?: string; width: number; height: number; durationSeconds?: number }) { const db = await getDb(); if (!db) return { id: 0, status: "pending" as const }; const permissions = await getUserMediaPermissions(userId); if (!permissions.reelsEnabled) return { id: 0, status: "disabled" as const }; const ratio = input.width / input.height; if (input.width < 1 || input.height < 1 || Math.abs(ratio - 9 / 16) > 0.03) return { id: 0, status: "invalid_ratio" as const }; const result = await db.insert(reelSubmissions).values({ userId, mediaUrl: input.mediaUrl, caption: input.caption || null, width: input.width, height: input.height, durationSeconds: input.durationSeconds ?? null }); const id = Number(result[0]?.insertId || 0); await addModerationAudit(userId, "reel_submitted", "reel", id, `9:16 submission ${input.width}x${input.height}`); return { id, status: "pending" as const }; }
...
export async function reviewReel(reelId: number, reviewerId: number, status: "approved" | "rejected", reviewNote?: string) { const db = await getDb(); if (!db) return { success: false }; await db.update(reelSubmissions).set({ status, reviewNote: reviewNote || null, reviewedBy: reviewerId, reviewedAt: new Date() }).where(eq(reelSubmissions.id, reelId)); await addModerationAudit(reviewerId, `reel_${status}`, "reel", reelId, reviewNote || null); return { success: true }; }
