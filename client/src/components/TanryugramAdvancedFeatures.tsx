...
export function CallOverlay({ callId, callType, isCaller, peer, onClose, onCallAgain }: { callId: number; callType: "audio" | "video"; isCaller: boolean; peer?: any; onClose: () => void; onCallAgain?: () => void }) {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true }, video: callType === "video" ? { facingMode: "user" } : false });
        if (cancelled) return;
        streamRef.current = stream;
        if (localVideo.current) localVideo.current.srcObject = stream;
        const pc = new RTCPeerConnection({ iceServers: [{ urls: "stun:stun.l.google.com:19302" }, { urls: "stun:stun1.l.google.com:19302" }, { urls: "stun:stun.cloudflare.com:3478" }], iceCandidatePoolSize: 10 });
        pcRef.current = pc;
        stream.getTracks().forEach((track) => pc.addTrack(track, stream));
        pc.ontrack = (event) => { if (remoteVideo.current) remoteVideo.current.srcObject = event.streams[0]; if (remoteAudio.current) { remoteAudio.current.srcObject = event.streams[0]; remoteAudio.current.volume = speaker ? 1 : 0.25; void remoteAudio.current.play().catch(() => undefined); } };
        pc.onicecandidate = () => { /* candidates are included after ICE gathering below */ };
        pc.onconnectionstatechange = () => { if (pc.connectionState === "connected" && remoteAudio.current) { remoteAudio.current.volume = speaker ? 1 : 0.25; void remoteAudio.current.play().catch(() => undefined); } };
        if (isCaller) {
        ...
        const message = error?.name === "NotFoundError" ? "No microphone or camera device is available on this phone." : error?.name === "NotAllowedError" ? "Allow TanRyuGram microphone and camera access in Android Settings, then try again." : error?.name === "NotReadableError" ? "The microphone is busy or unavailable. Close other calling or recording apps, then try again." : error?.message || "Check microphone and camera permissions.";
        setMediaError(message);
        toast.error("Could not start call", { description: message });
        await updateCall.mutateAsync({ callId, status: "ended", durationSeconds: 0 }).catch(() => undefined);
      }
  ...
  useEffect(() => { if (callQuery.data?.status === "accepted") { setMuted(false); if (remoteAudio.current) { remoteAudio.current.volume = speaker ? 1 : 0.25; void remoteAudio.current.play().catch(() => undefined); } } }, [callQuery.data?.status, speaker]);
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-zinc-950 text-white"><audio ref={remoteAudio} autoPlay playsInline controls={false} onLoadedMetadata={() => { if (remoteAudio.current) { remoteAudio.current.volume = speaker ? 1 : 0.25; void remoteAudio.current.play().catch(() => undefined); } }} className="absolute h-px w-px opacity-0" /><div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-violet-950 via-zinc-950 to-black p-6 text-center shadow-2xl">{callType === "video" && <><video ref={remoteVideo} autoPlay playsInline className="absolute inset-0 h-full w-full object-cover opacity-80" /><video ref={localVideo} autoPlay muted playsInline className="absolute right-4 top-4 h-32 w-24 rounded-2xl border border-white/30 object-cover" /></>}<div className="relative z-10"><div className="mx-auto mb-5 flex h-28 w-28 items-center justify-center rounded-full bg-white/15 ring-8 ring-white/5"><UserRound className="h-12 w-12" /></div><p className="text-2xl font-semibold">{peer?.name || peer?.username || (isCaller ? "Calling" : "Incoming call")}</p><p className="mt-2 text-sm text-white/60">{isCaller && callQuery.data?.status === "pending" ? "Calling…" : callQuery.data?.status === "pending" ? "Connecting…" : `${String(Math.floor(elapsed / 60)).padStart(2, "0")}:${String(elapsed % 60).padStart(2, "0")}`}</p><div className="mx-auto mt-6 flex items-center justify-center gap-1.5 opacity-60"><span className="h-2 w-2 animate-pulse rounded-full bg-violet-300" /><span className="h-3 w-3 animate-pulse rounded-full bg-violet-300 [animation-delay:120ms]" /><span className="h-2 w-2 animate-pulse rounded-full bg-violet-300 [animation-delay:240ms]" /></div></div><div className="absolute bottom-10 left-0 right-0 z-10 flex items-center justify-center gap-3"><button onClick={() => setMuted((value) => { const next = !value; streamRef.current?.getAudioTracks().forEach((track) => track.enabled = !next); return next; })} className={`rounded-full p-4 ${muted ? "bg-rose-500/80" : "bg-white/15"}`} title="Mute microphone">{muted ? <VolumeX className="h-5 w-5" /> : <Mic className="h-5 w-5" />}</button><button onClick={() => void toggleSpeaker()} className="rounded-full bg-white/15 p-4" title="Toggle speaker output">{speaker ? <Volume2 className="h-5 w-5" /> : <VolumeX className="h-5 w-5" />}</button>{callType === "video" && <><button onClick={toggleVideo} className={`rounded-full p-4 ${videoEnabled ? "bg-white/15" : "bg-rose-500/80"}`} title="Toggle video"><Video className="h-5 w-5" /></button><button onClick={flipCamera} className="rounded-full bg-white/15 p-4" title="Flip camera"><Camera className="h-5 w-5" /></button><button onClick={() => void togglePictureInPicture()} className="rounded-full bg-white/15 p-4" title="Picture in picture"><PictureInPicture className="h-5 w-5" /></button></>}<button disabled={ending} onClick={() => void end()} className="rounded-full bg-rose-500 p-4 disabled:opacity-60" title="End call"><PhoneOff className="h-5 w-5" /></button></div></div></div>;
}
...
export function AdvancedMessagesView() {
  ...
  const trpcUtilsUpdateCall = async (callId: number, status: "accepted" | "declined") => { await updateCall.mutateAsync({ callId, status }); window.dispatchEvent(new CustomEvent("tanryugram-call-state-changed", { detail: { callId, status } })); callsQuery.refetch(); };
